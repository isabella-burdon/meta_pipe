...
input_directory = 'c_qcmetrics'
folders = sorted(folder for folder in os.listdir(input_directory) if not folder.startswith('.'))

# loop must go into each of the folders of the directory to find the html file
for folder in folders:
    print(f"""
parsing quality metrics for {folder}""")
            
    html_path = os.path.join(input_directory, folder)
    for file in os.listdir(html_path):
        if file.endswith('.html'):
            ...
            # print to text file
            with open(f'c_qcmetrics/chopper_lengths/{folder}_nanoQClengths.txt', 'w') as f:
