...
run_name_list = sorted([os.path.splitext(os.path.basename(file))[0].replace('.fastq', '').replace('.gz', '') for file in os.listdir(your_data_directory) if file.endswith('.fastq.gz')])
...
for run_name in run_name_list:
    ...
    output_filepath = f"b_readsDepleted/{run_name}"
